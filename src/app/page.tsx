...
const genres = ['Electronic', 'Lo-Fi', 'Hip Hop', 'Ambient', 'Synthwave', 'Rock', 'Jazz', 'Indie', 'R&B', 'Pop'];
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchTracks = async () => {
      try {
          const response = await fetch('/api/tracks?limit=10&sort=popular');
          const data = await response.json();
          setTrendingTracks(data.tracks || []);
          
          const recentResponse = await fetch('/api/tracks?limit=5&sort=recent');
