    ...
    const handleTimeUpdate = () => {
      ...
      const handleEnded = () => {
        ...
        if (state.currentTrack) {
          const trackId = state.currentTrack.id || (state.currentTrack as any)._id;
          if (trackId) {
            fetch('/api/tracks/stream', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ trackId, duration: audio.duration || 0, completed: true }),
            }).catch(() => {});
          }
        }
      ...
      if (audioRef.current) {
        ...
        if (trackId) {
          fetch('/api/tracks/stream', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ trackId, duration: 0, completed: false }),
          }).catch(() => {});
        }
      }
    };
